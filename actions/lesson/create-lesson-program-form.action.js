...
import { createLessonProgram } from '@/actions/lesson/create-lesson-program.action';

export const createLessonProgramAction = async (state, formData) => {
    ...
    try {
        const response = await createLessonProgram(payload);

        const data = await response.json();

        if (!response.ok) {
            errorObject(
                'There was an error creating the education term!',
                data
            );
        }
